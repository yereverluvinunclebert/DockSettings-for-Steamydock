...
'---------------------------------------------------------------------------------------
'
Public Sub readDockSettingsFile(location As String, settingsFile As String)
...
Public Sub validateRegistryBehaviour()
   ...
   On Error GoTo validateRegistryBehaviour_Error
    ...
    If Val(sDAutoHideType) <= 0 And Val(sDAutoHideType) > 2 Or sDAutoHideType = "" Then sDAutoHideType = "0"
    
    If rDHotKeyToggle = "" Then rDHotKeyToggle = "F11" ' .02 STARTS DAEB 27/01/2021 Added validation of the function key used for fading the dock back in.
