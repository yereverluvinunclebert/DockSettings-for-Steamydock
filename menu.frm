...
Private Sub mnuAutoHide_Click()
   On Error GoTo mnuAutoHide_Click_Error
    ...
    If mnuAutoHide.Checked = True Then
        Call dock.staticRedrawLoop 'also calls this when the autohide timer has done its job
...
Private Sub mnuCloseApp_Click()
    ...
    If checkAndKill(NameProcess, True) = True Then ' .06 DAEB 05/03/2021 menu.frm Simplified the boolean checks and removed the cannot kill message
        ...
        If IsRunning(NameProcess, vbNull) = False Then ' .06 DAEB 05/03/2021 menu.frm Simplified the boolean checks and removed the cannot kill message
            ...
            initiatedProcessArray(selectedIconIndex) = "" ' removes the entry from the array that we test regularly so it isn't caught again
